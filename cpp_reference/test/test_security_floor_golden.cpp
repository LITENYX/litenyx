...
BOOST_AUTO_TEST_CASE(test_alpha_rounding) {
    ...
    input.b_fork = q64(29400000000ULL); // $294 in satoshi

    Q32_32 result = calculate_alpha(input);

    // Expected: α ≈ 1700.68
    // α = (1.45e12 / 2.9e12) × (1e11 / 2.94e10) = 0.5 × 3.401... = 1.70068...
    Q32_32 expected = q32(1700.68);

    // Allow reasonable tolerance for fixed-point
    BOOST_CHECK(q32_eq(result, expected, 1000000)); // ~0.0002 tolerance
}
...
BOOST_AUTO_TEST_CASE(test_b_min_rounding) {
    ...
    BOOST_CHECK(q64_eq(result, expected, 10000000000ULL)); // ~$1 tolerance
}
